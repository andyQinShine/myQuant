...
def get_symbol_balance(symbol):
    ...
    """
    balance = 0
    balanceList = get_balance()['data']['list']
...
# 得到交易对的价格和数量精度
def get_symbol_precision(base, quote):
    precision = get_symbols()['data']
    result = ''
...
def get_current_price(symbol):
    current_data = get_merged(symbol)
    return current_data['tick']['close']


if __name__ == '__main__':
    print (get_symbol_precision('btc','eos'))
